...
for p in pc_list:
    temp_proj_3d = TSNE(n_components=3,n_iter=1000,verbose=3).fit_transform(pca_data_array[:,:p])
    np.savetxt(os.path.join(out_dir,'ukbb_tsne_pc' + str(p) + '_plex30_iter_1000_3d'), temp_proj_3d)
    temp_proj_2d = TSNE(n_components=2,verbose=3).fit_transform(temp_proj_3d)
    np.savetxt(os.path.join(out_dir,'ukbb_tsne_pc' + str(p) + '_plex30_iter_1000_2d'), temp_proj_2d)
