...
\begin{abstract}
%Max 250
Background: Population structure in genetic data depends on complex demographic processes including geographic isolation, genetic drift, migration, and admixture. Existing data exploration methods struggle with unbalanced cohort ancestries, reconciliation of global versus local relationships, and the sheer size of modern datasets. We investigate an approach to dimension reduction and visualization of genomic data that combines principal components analysis (PCA) with uniform manifold approximation and projection (UMAP) to reveal layers of diversity and cryptic population structure.
\\
\\
Results: We demonstrate using genotype data from the 1000 Genomes Project, the Health and Retirement Study, and the UK Biobank that projections using PCA-UMAP effectively cluster individuals who are genetically closely related while placing them in a global continuum of genetic variation. These projections quickly reveal subpopulations within the American Hispanic population, deep relationships between geography and genotype in the UK population, and stark phenotypic variation between groups in the UK Biobank. 
\\
\\
