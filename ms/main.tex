...
\begin{abstract}
\adpcomment{Changed the abstract.}
Genetic structure in cohorts results from technical, sampling and demographic variation, necessitating visualization as an early step in genomic analyses. Existing approaches are stymied by the massive size of modern cohorts and unbalanced sizes of populations of many disparate ancestries and struggle to place individuals of admixed ancestries. They also struggle to reconcile local genetic relationships in a global context, and the push for larger and more diverse genomic data will only exacerbate these issues. We investigate an approach to dimension reduction of genomic data that combines principal components analysis (PCA) with uniform manifold approximation and projection (UMAP) to succinctly reveal cryptic population structure, the flow between layers of diversity, and phenotype heterogeneity between populations. Using data from large-scale genomic datasets, we demonstrate that PCA-UMAP intuitively clusters closely related individuals while placing them in a global continuum of genetic variation. We uncover previously overlooked subpopulations within the American Hispanic population and fine-scale relationships between geography, genotypes, and phenotypes in the UK population, opening new lines of investigation for demographic research and statistical genetics. We also demonstrate that in a massive and diverse cohort, UMAP outperforms state-of-the-art dimension reduction. Given its small computational cost and promising results, PCA-UMAP should become a standard approach to exploratory analysis of population-scale genetic datasets.
\end{abstract}
...
\adpcomment{Added this subsection}
\subsubsection*{Performance of t-SNE and UMAP}
To compare UMAP to t-SNE, we ran Python's sklearn implementation of t-SNE on the top 10 PCs of each dataset, once using the default initialization, and once initializing it with a two-dimensional UMAP embedding and skipping the early exaggeration phase of t-SNE (typically the first 250 iterations). The two approaches produce similar results in smaller datasets such as the 1KGP and HRS, with the main differences being in run-time and some qualitative factors such as clusters being more interconnected when using UMAP (figures~\ref{fig:supp_tsne_umap_compare_1kgp},~\ref{fig:supp_tsne_umap_compare_hrs}). Using the UKBB, it becomes clear that UMAP is far better equipped to handle massive genomic datasets with individuals from a variety of ancestral populations. Looking at the Kullback-Leibler divergence after 20,000 iterations, t-SNE's default initialization did not improve upon initialization with the UMAP embedding (figure~\ref{fig:supp_tsne_umap_compare_ukbb_graph}). Qualitatively the standard t-SNE initialization generated a large number of arbitrary clusters, while initializing t-SNE with UMAP preserved much of the large scale structure created by the UMAP embedding but broke the global continuity (figure~\ref{fig:supp_tsne_umap_compare_ukbb}). Given the many desirable properties of UMAP, such computational performance and intuitive placement of individuals within a global genetic continuum, we conclude that UMAP is a superior alternative to t-SNE for large and diverse genomic datasets.
...
\begin{figure}[!htb]
    ...
    \caption{Comparing the visualizations of UMAP, standard t-SNE, and t-SNE initialized with a UMAP projection, on the top 10 principal components of the 1KGP.}
...
\begin{figure}[!htb]
    ...
    \caption{Comparing the visualizations of UMAP, standard t-SNE, and t-SNE initialized with a UMAP projection, on the top 10 principal components of the HRS.}
...
\begin{figure}[!htb]
    ...
    \caption{Comparing the visualizations of UMAP, standard t-SNE, and t-SNE initialized with a UMAP projection, on the top 10 principal components of the UKBB.}
...
\begin{figure}[!htb]
    ...
    \caption{Comparing the error terms of standard t-SNE versus t-SNE initialized with a UMAP embedding and no early exaggeration. Done on the 1KGP dataset with 5000 iterations.}
...
\begin{figure}[!htb]
    ...
    \caption{Comparing the error terms of standard t-SNE versus t-SNE initialized with a UMAP embedding and no early exaggeration. Done on the HRS dataset with 5000 iterations.}
...
\begin{figure}[!htb]
    ...
    \caption{Comparing the error terms of standard t-SNE versus t-SNE initialized with a UMAP embedding and no early exaggeration. Done on the UKBB dataset with 20000 iterations.}
