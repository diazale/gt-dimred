...
By patching together many local genetic neighbourhoods, UMAP visualizations preserve aspects of both local and global structure. Methods such as t-SNE and LargeVis\cite{tang2016visualizing} are similarly motivated, with key differences in how they measure neighbours and which cost functions are used in optimizing low-dimensional embeddings: pairwise Gaussian and t-distributed approximations and the KL divergence in t-SNE; approximate nearest neighbours and a likelihood function in LargeVis; and fuzzy simplicial set membership and the cross-entropy in UMAP. Further practical and theoretical comparisons can be found in~\cite{2018arXivUMAP}. Hierarchical clustering of networks has also been successful in detecting fine-scale population structure, though it is instead motivated by partitioning data into subnetworks to then find stable subclusters\cite{han2017clustering}; in practice, one could carry this out by detecting clusters in data already dimensionally reduced using UMAP\cite{howtoUMAP}.
...
In contrast to PCA, UMAP has more adjustable parameters. Changing the PC cutoff, minimum distance, and number of neighbours can change characteristics of the visualizations. Using a minimal number of neighbours (e.g. 5) can result in the formation of disjoint clusters comprised of related family members (fig~\ref{fig:supp_1kgp_families}), and using a low minimum distance can result in clusters becoming more compact, losing visual detail. We used a minimum distance of 0.5 and 15 neighbours; however, default suggested parameters in UMAP (minimum distance of 0.1 and 15 neighbours) generally perform well across datasets. 
